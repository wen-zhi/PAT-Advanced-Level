...
* [1082 Read Number in Chinese](#1082-read-number-in-chinese)
...
问题并不复杂。主要在于如何控制零的输出。零的输出的有以下特点：

1. 当有连续多个 `0` 且最后一个 `0` 之后**是一个非零数字**时，需要输出一个 `ling`，比如 `1001`。
2. 如果只是连续多个 `0` 但最后一个 `0` 之后**没有非零数字**，则无需输出 `ling`，比如 `1000`。

因此只要在遇到非零数字时，检查上一位是否为 `0`，如果是，则输出 `ling`。

除此之外，还需要处理 `Wan` 的问题。比如：

- `101000000`，需要输出 `Wan`
- `100001000`，不需要输出 `Wan`

其规律为，当 `Wan` 所对应的位置及之前不是连续的 4 个零，就输出 `Wan`。
